...
| `x is T.Variant(a) && a > 0` outside an `if` | the guards that `if` would write, inside a `func() bool { … }()` answering whether they all held |
...
| `Result<T, E>`, `Table`, `hive.file.TableError` | provided by the runtime package |
